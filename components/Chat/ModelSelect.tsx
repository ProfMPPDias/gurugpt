...
export const ModelSelect = () => {
  ...
  function timeAgo(date: Date): string {
    ...
    if (days > 0) {
      return `${days} dia(s)${days > 1 ? 's' : ''} atrás`;
    } else if (hours > 0) {
      return `${hours} hora(s)${hours > 1 ? 's' : ''} atrás`;
    } else {
      return `${mins} minuto(s)${mins > 1 ? 's' : ''} atrás`;
    }
  }
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="mb-2 text-left text-neutral-700 dark:text-neutral-400">
        <p className='mt-2'>
          <span className="mr-16 inline-block">Tamanho:</span>
          <span className="inline-block">{selectedModelDetails.size}</span>
        </p>
        <p>
          <span className="mr-8 inline-block">Modificado á:</span>
